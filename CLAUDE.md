...
- **确定性压缩**:gzip/deflate/deflate_raw 级别 `flate.BestSpeed`、MTIME 归零;`br` 级别 `brotli.BestSpeed`(quality=0);`compress`(UNIX LZW/.Z)纯 Go 实现(`builder/compress_lzw.go`,无可用库),3 字节头 `[0x1F 0x9D 0x90]`(maxbits=16|块模式),LSB-first 连续位打包、码宽在 `freeEnt > maxcode+1` 时升档、表满(65536)冻结不发清除码(与 gunzip 的 uncompress 兼容)。
...
- 序列化纯函数 `builder.applyContentCodings` / `applyTransferCodings` / `chunkedFrame` / `applyAutoContentLength`(`builder/http_coding.go`);
  校验 `scenario.validateHTTPCodings`(`http_validate.go`);管线入口 `serializeHTTPReq`/`Resp` 的 `httpPayload`(`builder/http.go`)。compress LZW 原语 `builder.compressLZW`(`compress_lzw.go`,被 `compressCoding` 接线至 CE/TE fold)。
