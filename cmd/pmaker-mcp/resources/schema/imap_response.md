...
| `status` | string | 状态组必填 | `OK`/`NO`/`BAD`/`PREAUTH`/`BYE`(大小写不敏感,原样输出);与 `data` 互斥;非标状态走 `payload`/`payload_hex` |
...
| `data` | string | 数据组必填 | untagged 数据文本(如 `FLAGS (...)`、`42 EXISTS`、`1 FETCH (BODY[] `);**首 token 不能是 `OK`/`NO`/`BAD`/`PREAUTH`/`BYE`**(否则与状态组歧义,走 status 组);**不能含 `\r` / `\n`** |
