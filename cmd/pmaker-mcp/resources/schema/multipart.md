...
boundary 必须与父层 `Content-Type` 头里的 `boundary=` 参数一致(不一致会产出告警,见「一致性告警」)。
...
| `encoding` | string | 否 | 传输编码(RFC 2045 §6 CTE):`none`(缺省)/`7bit`/`8bit`/`binary`/`base64`/`quoted-printable`。`none`/`7bit`/`8bit`/`binary` 为恒等编码(透传,仅声明 body 字节性质、不做变换);`base64`/`quoted-printable` 为真变换。设非 `none` 时建议配对应的 `Content-Transfer-Encoding` 头(不一致会产出告警,见「一致性告警」) |
...
- 每 part body 先取字节(`body` 或 `body_hex`),再按 `encoding` 编码:
  - `base64`:标准 base64 字母表,按 RFC 2045 **每 76 字符折行**(`\r\n` 分隔,输出确定性);
  - `quoted-printable`:RFC 2045 §6.7 quoted-printable;
...
- EML 下 multipart 字节即邮件正文,作用顺序固定为「先按 `encoding` 编码 → 再按 boundary 拼装」;
  dot-stuffing 与终止符等成帧字节由承载它的 SMTP/POP3 在最外层自动追加(见 `pmaker://schema/eml_data`)。
...
- **boundary 碰撞**(生成 pcap 时检查):part 编码后 body 里若出现独占一行的 `--<boundary>`(RFC 2046 §5.1.1:分界符须独占一行),解析端会误判切分 multipart → 告警。`@file` 注入附件时尤其隐蔽;命中时请更换更长的 boundary(默认 boundary 碰撞概率极低)。
